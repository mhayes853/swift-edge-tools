...
if [[ "$INSTALL_NEEDLE2_RUNTIME" == 1 ]]; then
  # Needle 2 calls `std::__1::__hash_memory`, which only libc++ 21 and newer export.
  ...
  apt-get update
  DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends libc++-21-dev libc++abi-21-dev
  export LIBRARY_PATH="/usr/lib/llvm-21/lib${LIBRARY_PATH:+:$LIBRARY_PATH}"
fi
