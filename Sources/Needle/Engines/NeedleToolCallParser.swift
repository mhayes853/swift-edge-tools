...
public struct NeedleToolCallParser: Sendable {
  ...
  public mutating func accept(token: NeedleToken) -> NeedleRawToolCall? {
  ...
  private mutating func handleOutsideBlock() -> NeedleRawToolCall? {
  ...
  private mutating func handleInsideArray() -> NeedleRawToolCall? {
  ...
  private func shouldTreatAsBoundary(_ character: Character) -> Bool {
    ...
    return character.isNeedleToolCallBoundary
  }
  ...
  ) -> (closed: Bool, call: NeedleRawToolCall?) {
  ...
  private func parse(objectString: String) -> NeedleRawToolCall? {
    guard let data = objectString.data(using: .utf8) else { return nil }
    return try? JSONDecoder().decode(NeedleRawToolCall.self, from: data)
  }
}

extension Character {
  fileprivate var isNeedleToolCallBoundary: Bool {
    ["{", "}", "]"].contains(self)
  }
}
