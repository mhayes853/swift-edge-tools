...
struct `Needle2JSEngine tests` {
  ...
  func `Generates Real Tool Calls Across Context Reset`() async throws {
    ...
    #expect((generation.engineGeneration.metrics.decodeTokens ?? 0) > 0)
    #expect(generation.engineGeneration.metrics.needle2ResponseType == "call")
    #expect(generation.engineGeneration.metrics.prefillTokensPerSecond != nil)
    #expect(generation.engineGeneration.metrics.decodeTokensPerSecond != nil)
    #expect(generation.engineGeneration.metrics.needle2PeakRAMMegabytes == nil)
