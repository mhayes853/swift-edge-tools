  ...
  @Suite
  struct `NeedleONNXEngine core tests` {
    @Test
    func `Generate Throws When Custom Backend Returns Invalid Logits`() async throws {
      let tokenizer = try makeTestTokenizer()
      let engine = try NeedleONNXEngine(
        backend: InvalidLogitsNeedleONNXBackend(),
        tokenizer: tokenizer
      )

      let error = await #expect(throws: EdgeToolsONNXError.self) {
      ...
      expectNoDifference(error?.code, .invalidLogitsCount)
    }

    @Test
    func `Generate Uses Custom ONNX Backend`() async throws {
      let tokenizer = try makeTestTokenizer()
      let engine = try NeedleONNXEngine(
        backend: FailingNeedleONNXBackend(),
        tokenizer: tokenizer
      )

      await #expect(throws: FailingNeedleONNXBackend.Failure.self) {
        let generationTask = try engine.generate(
          prompt: NeedlePrompt(system: "", user: "Use the custom backend."),
          parameters: .default,
          channel: EdgeToolsGenerationChannel()
        )
        _ = try await generationTask.value
  ...
  private struct InvalidLogitsNeedleONNXBackend: NeedleONNXBackend {
    let configuration = NeedleModelConfiguration()

    func prefill(
      tokenIDs: [EdgeToolsToken.ID]
    ) async throws -> any NeedleONNXGeneration {
      InvalidLogitsNeedleONNXGeneration()
    }
  }

  private struct InvalidLogitsNeedleONNXGeneration: NeedleONNXGeneration {
    func decode(
  ...
  private struct FailingNeedleONNXBackend: NeedleONNXBackend {
