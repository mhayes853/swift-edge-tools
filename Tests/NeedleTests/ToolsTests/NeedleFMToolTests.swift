  ...
  struct `NeedleFMTool tests` {
    ...
    @Test
    @available(iOS 26.0, macOS 26.0, watchOS 26.0, tvOS 26.0, visionOS 26.0, *)
    func `Preserves Needle Value And Typed Arguments`() throws {
      let input = try NeedleFMToolInput<WeatherArgs>(
        needleValue: ["city": "Brooklyn", "units": "metric"]
      )

      expectNoDifference(input.needleValue, ["city": "Brooklyn", "units": "metric"])
